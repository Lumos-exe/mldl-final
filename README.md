...
本项目是《机器学习与深度学习》课程设计，选题对应老师给出的“卷积神经网络与 ViT 结合的探索与实现”。项目以 CIFAR-100 图像分类为任务，实现轻量 CNN、轻量 ResNet-Transformer Hybrid、CIFAR 版 ResNet-18、强 Hybrid 和 Small ViT，并通过严格消融实验研究轻量 Hybrid 中 attention 机制、Transformer 层数和 patch size 的影响。
...
CNN 的优势在于局部归纳偏置强，训练稳定，适合小图像任务；ViT 的优势在于 self-attention 能建模较远区域之间的关系，但在小数据集上可能更依赖正则化和训练技巧。混合模型以 ResNet-18 为主干，在第二阶段后的 $16\times16$ 特征和第三阶段后的 $8\times8$ 特征上各插入一个 MobileViT 风格的局部-全局-局部融合块：先用卷积提炼局部表示，再通过 unfold/fold 将特征组织为 patch token 输入 Transformer 建模全局关系，最后把 token 无损还原成特征图，并通过卷积与原 CNN 特征融合后继续送入后续 ResNet 阶段。这样做不是用 stride patch embedding 压缩局部细节，也不是只在最终分类头前“过一下”，而是让全局关系建模多次参与卷积特征提取。

为了避免“谁参数更多谁占便宜”，本项目采用参数预算匹配。轻量组中，`compact_cnn` 使用 32 宽 ResNet，约 2.82M 参数；`compact_hybrid` 使用 27 宽 ResNet 主干和双阶段 MobileViT 风格融合块，约 2.9M 参数。强组中，`cnn_baseline` 是 64 宽 ResNet-18，`hybrid_main` 使用 54 宽主干和双阶段融合块，约 11.6M 参数。这样做不是简单给 CNN 额外加 Transformer，而是在相近参数预算下重新分配 CNN 与 Transformer 的容量。
...
主实验分为轻量组和强基线组。轻量组是主要观察对象，用来判断容量受限时 Transformer 融合是否更明显；强基线组用来判断当 CNN 已经较强时，Hybrid 的收益是否变小。
